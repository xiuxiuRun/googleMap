...
if(session.getAttribute("Latitude")==null){
	session.setAttribute("Latitude",117.156586f);
}
//上右延长
Float Longitude = (Float)session.getAttribute("Longitude");//经度
Float Latitude = (Float)session.getAttribute("Latitude");//纬度
Float neweLongitude = Longitude+0.05f;
Float newLatitude = Latitude;

if(neweLongitude>53.55f){
	try{
		File tofile=new File("C:/Users/dell2/Desktop/1234.txt");
...
<html>
  ...
  function initialize() {
    ...
    if(parseFloat(newLatitudeT) > parseFloat(126.156586)){
    	alert("out of border");
    	alert("右上角已经下载完毕");
    }
